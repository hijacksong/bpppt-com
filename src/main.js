...
const pricing = [
  {
    ...
    price: '¥0',
    desc: '用免费工具建立信任和获客。',
    items: ['BP 大纲生成', '基础结构检查', '3 个免费页面模板', '融资材料清单'],
    action: '免费开始',
  },
  {
    name: 'Pro',
    price: '¥99/月',
    desc: '适合正在制作 BP 的创业者。',
    items: ['行业模板库', 'AI 页面文案优化', 'PPT/PDF 导出', '中英双语改写'],
    action: '升级 Pro',
    highlight: true,
  },
  {
    name: 'Founder Review',
    price: '¥699/次',
    desc: '高价值人工审阅，避免只卖低价模板。',
    items: ['整份 Deck 诊断', '投资人问题清单', '页面删改建议', '30 分钟顾问反馈'],
    action: '预约审阅',
  },
]
...
function renderScenarioPanel(item = scenarios[0]) {
  return `
    ...
    <a class="button primary" href="#contact">${item.cta}</a>
  `
}
  ...
  <main id="top">
      ...
      <div class="section-shell dark-content">
        ...
        <div class="capability-grid">
          ${capabilities.map(([num, title, body]) => `
            <article><span>${num}</span><strong>${title}</strong><p>${body}</p></article>
          `).join('')}
        </div>
      </div>
    </section>
    ...
    <section class="section-shell industries">
      ...
      <div class="industry-grid">
        ${industryDecks.map(([name, detail]) => `
          <article><h3>${name}</h3><p>${detail}</p><a href="#contact">获取 ${name} 结构 →</a></article>
        `).join('')}
      </div>
    </section>
      ...
      <div class="pricing-grid">
        ${pricing.map(plan => `
          <article class="price-card ${plan.highlight ? 'highlight' : ''}">
            ...
            <a class="button ${plan.highlight ? 'primary' : 'ghost'}" href="#contact">${plan.action}</a>
          </article>
        `).join('')}
      </div>
...
function updateScenario(id) {
  ...
  document.querySelectorAll('.scenario-card').forEach(card => {
    card.classList.toggle('active', card.dataset.scenario === selected.id)
  })
...
document.querySelectorAll('.scenario-card').forEach(card => {
  card.addEventListener('click', () => updateScenario(card.dataset.scenario))
})
