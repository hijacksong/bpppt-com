  ...
  <main id="top">
    ...
    <section id="scenarios" class="section-shell split-section">
      ...
      <div class="scenario-layout">
        <div class="scenario-grid">${o()}</div>
        <article class="scenario-result" id="scenarioResult">${s()}</article>
      </div>
    </section>
    ...
    <section id="value" class="dark-band">
      <div class="section-shell dark-content">
        ...
        <div class="capability-grid">
          ${t.map(([e,t,n])=>`<article><span>${e}</span><strong>${t}</strong><p>${n}</p></article>`).join(``)}
        </div>
      </div>
    </section>
    ...
    <section class="section-shell industries">
      ...
      <div class="industry-grid">
        ${n.map(([e,t])=>`<article><h3>${e}</h3><p>${t}</p><a href="#diagnosis">获取 ${e} 结构 →</a></article>`).join(``)}
      </div>
    </section>
    ...
    <section id="review" class="section-shell review-section">
      ...
      <div class="review-tool" aria-label="BP 诊断演示工具">
        ...
        <div class="checklist" id="scoreChecklist">${c()}</div>
      ...
      <div class="pricing-grid">
        ${r.map(e=>`
          <article class="price-card ${e.highlight?`highlight`:``}">
            <p>${e.name}</p><h3>${e.price}</h3><small>${e.desc}</small>
            <ul>${e.items.map(e=>`<li>${e}</li>`).join(``)}</ul>
            <a class="button ${e.highlight?`primary`:`ghost`}" href="${e.href}">${e.action}</a>
          </article>
        `).join(``)}
      </div>
...
`;function u(e){return String(e||``).replace(/[&<>'"]/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,"'":`&#39;`,'"':`&quot;`})[e])}function d(t){let n=e.find(e=>e.id===t)||e[0];document.querySelectorAll(`.scenario-card`).forEach(e=>e.classList.toggle(`active`,e.dataset.scenario===n.id)),document.querySelector(`#scenarioResult`).innerHTML=s(n)}function f(){let e=[...document.querySelectorAll(`input[name="score"]:checked`)],t=Object.keys(i).filter(t=>!e.some(e=>e.value===t)),n=Math.round(e.length/Object.keys(i).length*100);document.querySelector(`#scoreValue`).textContent=n,document.querySelector(`#scoreLabel`).textContent=n>=84?`Investor-ready`:n>=50?`Needs improvement`:`High risk deck`,document.querySelector(`#missingBox`).innerHTML=t.length?`<strong>建议补强：</strong>${t.map(e=>i[e][0]).join(`、`)}。这些缺口会影响投资人快速判断项目价值。`:`结构完整。下一步可以优化视觉表达和路演讲稿。`}function p(e){e.preventDefault();let t=e.currentTarget,n=new FormData(t),r=u(n.get(`project`)||`你的项目`),o=n.get(`industry`)||`ai`,s=n.get(`stage`),c=n.get(`goal`),l=Number(n.get(`pages`)||0),d=n.getAll(`module`),f=Object.keys(i).filter(e=>!d.includes(e)),p=42+d.length*8;s===`traction`&&(p+=6),s===`fundraising`&&(p+=3),c===`investor`&&!d.includes(`ask`)&&(p-=8),l>20&&(p-=6),p=Math.max(28,Math.min(96,p));let[m,h]=a[o],g=p>=80?`接近 Investor-ready`:p>=60?`需要补强后再见投资人`:`当前更适合先重构大纲`,_=f.length?f.map(e=>i[e][0]).join(`、`):`暂无核心缺口`,v=c===`investor`?`Problem → Solution → Why now → Market → Product → Traction → Business Model → GTM → Team → Ask`:`Problem → User → Solution → Market → Business Model → Validation Plan → Roadmap → Team`;document.querySelector(`#diagnosisResult`).innerHTML=`
    <p class="eyebrow">Free diagnosis result</p>
    <div class="result-top"><strong>${p}</strong><span>/100</span><small>${g}</small></div>
    <h3>${r} 的 BP 初步诊断</h3>
    <ul class="result-list">
      <li><b>行业：</b>${m}。${h}</li>
      <li><b>主要缺口：</b>${_}。</li>
      <li><b>页数提醒：</b>${l>20?`当前页数偏多，建议压缩成 10-15 页投资人版本。`:`当前页数可控，建议优先补齐核心逻辑。`}</li>
      <li><b>推荐页序：</b>${v}。</li>
    </ul>
  ...
  `}document.querySelectorAll(`.scenario-card`).forEach(e=>e.addEventListener(`click`,()=>d(e.dataset.scenario))),document.querySelectorAll(`input[name="score"]`).forEach(e=>e.addEventListener(`change`,f)),document.querySelector(`#diagnosisForm`).addEventListener(`submit`,p),f();